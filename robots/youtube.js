...
async function robot () {
  ...
  async function authenticateWithOAuth () {
    ...
    function requestUserConsent (OAuthClient) {
      ...
      console.log(`> Por favor, de seu consentimento: ${consentUrl}`)
      require('child_process').exec(`start "" "${consentUrl}"`)
    }

    async function waitForGoogleCallback (webServer) {
      return new Promise((resolve, reject) => {
        console.log('> Esperando consentimento do usuário ...')

        webServer.app.get('/oauth2callback', (req, res) => {
          const authCode = req.query.code
          console.log(`> Consentimento dado: ${authCode}`)

          res.send('<h1>Obrigado!</h1><p>Pode fechar esta aba.</p>')
          resolve(authCode)
        })
      })
    }
    ...
    async function requestGoogleForAccessTokens (OAuthClient, authorizationToken) {
      return new Promise((resolve, reject) => {
        OAuthClient.getToken(authorizationToken, (error, tokens) => {
          ...
          console.log('> Token de acesso concedido:')
          console.log(tokens)
  ...
  async function uploadVideo (content) {
    ...
    console.log(`> Video disponivel em: https://youtu.be/${youtubeResponse.data.id}`)
